...
- **Publicación de GitHub Pages corregida — 2026-09-12:** se detectó después de publicar `main` que el workflow de Pages seguía escuchando únicamente `codex/world-memories-mvp`, por lo que la PWA instalada continuaba recibiendo la versión histórica. El disparador se trasladó a `main`; la ejecución remota 34719547939 terminó correctamente con instalación, typecheck, pruebas, build y `deploy-pages`. La URL pública `https://hbarcenas88.github.io/world-memories-crm-pilot/` devuelve el mismo HTML y recursos que la build final local. El service worker conserva actualización bajo consentimiento: detecta una versión al reabrir/con conexión o en su revisión periódica, y la aplicación solicita aplicar la actualización en vez de activarla silenciosamente.

- **Cierre de aceptación y addendum de interfaz — 2026-09-12:** la usuaria confirmó el recorrido manual real en **Microsoft Edge a 100 % y 200 %**; esa evidencia sustituye la pausa histórica de zoom de esta misma fecha. Se aplicaron además cinco ajustes posteriores: el selector telefónico muestra país, ISO y lada, mientras residencia muestra sólo país; capas flotantes se descartan al hacer clic fuera; Correo conserva altura compacta; el rango conocido de Lead se ubica antes de presupuesto/moneda; y la regla se auditó en menú de acciones, búsqueda, calendario y notificaciones. `npm test -- --reporter=json --outputFile artifacts/vitest-final-2026-09-12-publish.json` pasó con **206 suites / 101 archivos / 434 pruebas**; `npm run test:e2e` pasó **29/29**; typecheck, lint, build PWA (14 recursos, 1,300.61 KiB), `npm audit --omit=dev --audit-level=high` (0 vulnerabilidades) y `git diff --check` pasaron. La advertencia de dos chunks >500 kB es deuda de rendimiento no bloqueante. El revisor independiente Astra Medium otorgó **GO para main** sin hallazgos Critical ni Important; se añadió además la aserción de foco destino para las capas descartables. La rama `main` se creó y publicó sin fuerza desde `e5a9ee1` el 12/09/2026. No se modificó la configuración de GitHub Pages.
