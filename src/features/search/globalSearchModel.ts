import type { Client, Commission, DeletedRecordReference, Lead, Provider, RichNote, Service, ServiceProvider, Task, Trip } from '../../domain/types';
...
type SearchWorkspace = Readonly<{ clients: readonly Client[]; leads: readonly Lead[]; trips: readonly Trip[]; providers: readonly Provider[]; tasks: readonly Task[]; commissions: readonly Commission[]; deletedReferences?: readonly DeletedRecordReference[]; notes?: readonly RichNote[]; services?: readonly Service[]; serviceProviders?: readonly ServiceProvider[] }>;
...
export function searchWorkspace(queryText: string, workspace: SearchWorkspace): readonly SearchResult[] {
  ...
  if (!query) return [];
  const tripClientLabel = (trip: Trip): string => {
    const resolvedClient = resolveRecordReference(
      workspace.clients,
      workspace.deletedReferences ?? [],
      'client',
      trip.clientId,
    );
    return resolvedClient.state === 'live'
      ...
      : resolvedClient.state === 'deleted'
        ? `Registro eliminado: ${resolvedClient.reference.displayLabel}`
  ...
  return [
    ...
    ...workspace.trips.filter((trip) => matches(query, tripClientLabel(trip), trip.effectiveStartOn, trip.effectiveEndOn, notesByTripId.get(trip.id), ...(locatorsByTripId.get(trip.id) ?? []))).map((trip) => ({ id: trip.id, kind: 'trip' as const, label: `Viaje: ${tripClientLabel(trip)}`, ...(notesByTripId.get(trip.id) ? { context: notesByTripId.get(trip.id) } : locatorsByTripId.get(trip.id)?.[0] ? { context: locatorsByTripId.get(trip.id)?.[0] } : {}) })),
