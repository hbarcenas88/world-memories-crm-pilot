...
import { matchesArchive } from '../records/archiveVisibility';
...
export function ProviderList({ providers, onSelect }: { providers: readonly Provider[]; onSelect: (provider: Provider) => void }) {
  ...
  const filteredProviders = providers.filter((provider) => {
    const isInArchiveFilter = matchesArchive(provider, archiveFilter);
    ...
    return isInArchiveFilter && matchesType && (normalizedQuery === '' || searchable.includes(normalizedQuery));
  });

  return <div className="lead-layout">
    <ArchiveFilterChips onChange={setArchiveFilter} value={archiveFilter} />
    <FilterBar label={label('filterProviders')} onClear={() => { setQuery(''); setServiceType(''); }}><div className="form-grid task-filters">
