...
export function ClientDetail({ client, trips, leads = [], events, onOpenTrip, onOpenLead, onClose, onOpenWorkspace, recordActions }: ClientDetailProps) {
  ...
  return <aside aria-label={label('clientWorkspace')} className="client-detail">
    <RecordHeader actions={<>{onOpenWorkspace && <button className="secondary-button" onClick={onOpenWorkspace} type="button">{label('openFullWorkspace')}</button>}{recordActions}<IconButton label={label('closeClient')} onClick={onClose}><X aria-hidden="true" /></IconButton></>} eyebrow={label('clientFamily')} title={client.name} />
    ...
    <AccordionSection summary={String(timeline.length)} title={label('aggregateHistory')}>{timeline.length === 0 ? <p className="muted-copy">{label('noLinkedEvents')}</p> : <ol className="timeline">{timeline.map((event) => <li key={event.id}><span aria-hidden="true">●</span><div><strong>{activityEventLabel(event.type, locale)}</strong><small>{event.aggregateType === 'lead' ? label('lead') : event.aggregateType === 'trip' ? label('trip') : label('client')} · {formatOperationalDateTime(event.occurredAt)}</small></div></li>)}</ol>}</AccordionSection>
  </aside>;
}
