...
import type { BackupDownload, Client, Commission, DeletedRecordReference, Lead, Payment, Service, ServiceProvider, Task, Trip } from '../../domain/types';
...
type DashboardProps = Readonly<{ today: string; leads: readonly Lead[]; trips: readonly Trip[]; tasks: readonly Task[]; commissions: readonly Commission[]; clients?: readonly Client[]; deletedReferences?: readonly DeletedRecordReference[]; services?: readonly Service[]; serviceProviders?: readonly ServiceProvider[]; payments?: readonly Payment[]; backupDownloads?: readonly BackupDownload[]; onOpenTrip?: (tripId: string) => void; onOpenTask?: (taskId: string) => void; onOpenCommission?: (commissionId: string) => void; onCompleteTask?: (taskId: string) => void; onRescheduleTask?: (taskId: string, dueOn: string) => void | Promise<void> }>;

export function Dashboard({ today, leads, trips, tasks, commissions, clients = [], deletedReferences = [], services = [], serviceProviders = [], payments = [], backupDownloads = [], onOpenTrip, onOpenTask, onOpenCommission, onCompleteTask, onRescheduleTask }: DashboardProps) {
  const locale = useLocale();
  const snapshot = buildDashboardSnapshot({ today, leads, trips, tasks, commissions, services, serviceProviders, payments });
  const backupReminder = buildBackupReminder(backupDownloads, `${today}T12:00:00.000Z`);
  const tripClientLabel = (trip: Trip) => {
    const resolvedClient = resolveRecordReference(clients, deletedReferences, 'client', trip.clientId);
    return resolvedClient.state === 'live'
  ...
  return <section aria-label={t('operationalDashboard', locale)} className="dashboard-grid">
    ...
    <section className="dashboard-queue dashboard-queue-wide"><h2>{t('priorityAttention', locale)}</h2>{snapshot.overdueTasks.length === 0 ? <p className="muted-copy">{t('noOverdueTasks', locale)}</p> : <ul>{snapshot.overdueTasks.map((task) => <li key={task.id}><strong>{task.title}</strong><span>{t('overdueOn', locale, { date: task.dueOn ? formatOperationalDate(task.dueOn) : t('undefinedValue', locale) })}</span><div className="task-actions">{onOpenTask && <button className="text-button" onClick={() => onOpenTask(task.id)} type="button">{t('openTask', locale)}</button>}{onRescheduleTask && <TaskRescheduleControl onReschedule={onRescheduleTask} task={task} />}{onCompleteTask && <button className="secondary-button" onClick={() => onCompleteTask(task.id)} type="button">{t('complete', locale)} {task.title}</button>}</div></li>)}</ul>}</section>
